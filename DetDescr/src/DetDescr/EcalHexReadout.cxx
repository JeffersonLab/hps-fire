...
EcalHexReadout::EcalHexReadout(const Parameters &ps)
    : ConditionsObject(EcalHexReadout::CONDITIONS_OBJECT_NAME) {
...
void EcalHexReadout::buildCellMap() {
  ...
  int ecalMapID = 0; // ecalMap cell IDs go from 0 to N-1, not equal to original
                     // grid cell ID.
...
void EcalHexReadout::buildNeighborMaps() {
  ...
  if (verbose_ > 2) {
    ...
    std::cout << TString::Format("This bin is a distance of %.2f away from a "
                                 "module edge. Decision isEdge %d.",
...
bool EcalHexReadout::isInside(double normX, double normY) const {
  ...
  if (verbose_ > 2)
    std::cout << TString::Format("[isInside] they are inside quadrant. Dot "
                                 "product (>0 is inside): %.2f ",
                                 dotProd)
              << std::endl;
  return (dotProd > 0.);
}
