...
public:
  ...
  int getCellIDRelative(double x, double y) const {
    ...
    if (bin < 0) {
      TString error_msg =
          TString("[EcalHexReadout::getCellIDRelative] Relative coordinates "
                  "are outside module hexagon!") +
          TString::Format(" Is the gap used by EcalHexReadout (%.2f mm) and "
                          "the minimum module radius (%.2f mm)",
                          gap_, moduler_) +
          TString::Format(" the same as hexagon_gap and Hex_radius in "
  ...
  std::vector<EcalID> getNN(EcalID id) const {
    ...
    for (auto &flat : list)
      id = EcalID(id.layer(), flat.module(), flat.cell());
    return list;
  }
   ...
   */
  const std::map<EcalID, std::pair<double, double>> &
  getCellModulePositionMap() const {
